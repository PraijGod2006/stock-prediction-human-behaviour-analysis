...
- Position Sizer: ATR-adjusted volatility sizing (calm stocks get larger allocations,
...
def load_models() -> tuple:
    ...
    if not os.path.exists(MODEL_1_PATH) or not os.path.exists(MODEL_2_PATH) or not os.path.exists(MODEL_3_PATH):
...
def generate_signals(
    ...
    for i in range(min_len):
        ...
        # Position sizing via ATR formula
        shares = 0.0
        position_value = 0.0
        exposure_allowed = True
        
        if signal_type != "HOLD":
            # Check exposure limits from correlation matrix
