...
- Section 1: Mid-price used everywhere, 14 z-score formulas, session resets.
- Section 2: Lagged cross-asset returns joined via Pandas datetime index.
- Section 3: All 3 models regularized, early stopping active, targets defined.
- Section 4: Incremental streaming, xgb_model passed, memory released.
- Section 7: PurgedWalkForwardCV wired to company train/val splits.
- Section 8: Replay buffer populated and mixed for both 5-min and 1-min.
- Section 12: PSI drift detection and training logger active.
- Section 13: Model 2 target matches exact 1-min bar of next window.
========================================================================================
"""
...
def _safe_write_parquet(df_polars: pl.DataFrame, path: str):
    """
    Writes a Polars DataFrame to Parquet safely on Windows.
    Uses a temp file + atomic rename to avoid file lock errors (OS error 1224).
    """
    temp_path = path + '.tmp'
...
def update_replay_buffer_5min(
    ...
    sample['__target_dir'] = y_dir.iloc[idx].values
    sample['__target_price'] = y_price.iloc[idx].values
...
def update_replay_buffer_1min(
    ...
    sample['__target_exhaust'] = y_exhaust.iloc[idx].values
    sample['__symbol'] = symbol
...
def load_replay_5min() -> Optional[tuple]:
    ...
    return buffer[feature_cols], buffer['__target_dir'], buffer['__target_price']


def load_replay_1min() -> Optional[tuple]:
    ...
    feature_cols = [c for c in buffer.columns if c not in meta_cols]
    return buffer[feature_cols], buffer['__target_exhaust']
...
def clean_data(X: pd.DataFrame, y: pd.Series):
    ...
    valid = X_clean.notna().all(axis=1).values & y_clean.notna().values
    return X_clean[valid], y_clean[valid]


def train_all_companies(tune_first_company: bool = False):
    """
    Main training loop across all 100 companies.
    """
    ...
    for i, parquet_path in enumerate(parquet_files):
        ...
        # Model 1 & 2 Targets: perfectly aligned to the exact same 5-minute index
        y_dir_raw = model_1.prepare_target(df_5min)['target'].rename('target_dir')
        y_price_raw = model_2.prepare_target(df_5min, df_1min=df_1min)['target'].rename('target_price')

        targets_5m = pd.concat([y_dir_raw, y_price_raw], axis=1).dropna()
        common_idx_5m = df_5min.index.intersection(targets_5m.index)

        X_5min = df_5min.loc[common_idx_5m, [c for c in df_5min.columns if c not in drop_cols]]
        y_dir = targets_5m.loc[common_idx_5m, 'target_dir']
        y_price = targets_5m.loc[common_idx_5m, 'target_price']

        # Clean 5-min features and targets together
        X_clean_5m = X_5min.replace([np.inf, -np.inf], np.nan).dropna(axis=1, how='all')
        y_dir_clean = y_dir.replace([np.inf, -np.inf], np.nan)
        y_price_clean = y_price.replace([np.inf, -np.inf], np.nan)
        valid_5m = X_clean_5m.notna().all(axis=1) & y_dir_clean.notna() & y_price_clean.notna()

        X_5min = X_clean_5m[valid_5m]
        y_dir = y_dir_clean[valid_5m]
        y_price = y_price_clean[valid_5m]

        # Model 3: Maximum 10-minute forward drawdown
        df_1min_with_target = model_3.prepare_target(df_1min)
        y_exhaust = df_1min_with_target['target']
        X_1min = df_1min_with_target[[c for c in df_1min_with_target.columns if c not in drop_cols]]
        X_1min, y_exhaust = clean_data(X_1min, y_exhaust)
        ...
        del df, df_1min, df_5min, targets_5m, df_1min_with_target
        gc.collect()
        ...
        # Section 7: Enforces strict purge and embargo gaps between train & validation folds
        # Fold 4 of 4 splits puts ~80% in training, purges 10 rows, embargos 5 rows, validates on remaining 20%
        cv_5m = PurgedWalkForwardCV(n_splits=4, purge_gap=10, embargo_gap=5)
        folds_5m = list(cv_5m.split(X_5min_train))
        train_idx_5m, val_idx_5m = folds_5m[-1]  # Latest fold for out-of-sample test

        X_tr5, X_va5 = X_5min_train.iloc[train_idx_5m], X_5min_train.iloc[val_idx_5m]
        y_tr_dir, y_va_dir = y_dir_train.iloc[train_idx_5m], y_dir_train.iloc[val_idx_5m]
        y_tr_price, y_va_price = y_price_train.iloc[train_idx_5m], y_price_train.iloc[val_idx_5m]
        ...
        X_tr1, X_va1 = X_1min_train.iloc[train_idx_1m], X_1min_train.iloc[val_idx_1m]
        y_tr_exh, y_va_exh = y_exhaust_train.iloc[train_idx_1m], y_exhaust_train.iloc[val_idx_1m]
        ...
        try:
            ...
            acc = accuracy_score(y_va_dir, preds)
        ...
        try:
            ...
            rmse_price = np.sqrt(mean_squared_error(y_va_price, y_pred_price))
        ...
        try:
            ...
            rmse_exh = np.sqrt(mean_squared_error(y_va_exh, y_pred_exh))
        ...
        # --- STEP 9: Periodic Full Retrain from Full Replay Buffer (every 25 companies) ---
        if (i + 1) % 25 == 0 and i > 0:
            ...
            try:
                ...
                replay_5 = load_replay_5min()
                if replay_5:
                    Xr, yr_dir, yr_price = replay_5
                    Xr = Xr.replace([np.inf, -np.inf], np.nan).dropna()
                    yr_dir = yr_dir.loc[Xr.index]
                    sp = int(len(Xr) * 0.8)
                    model_1_fresh = DirectionalModel()
                    model_1_fresh.train(Xr.iloc[:sp], yr_dir.iloc[:sp], Xr.iloc[sp:], yr_dir.iloc[sp:])
                    model_1_fresh.save(MODEL_1_PATH)
                    model_1 = model_1_fresh
                    print("  Model 1 successfully retrained from full replay buffer.")
                    del replay_5, Xr, yr_dir, yr_price

                # Retrain Model 3
                replay_1 = load_replay_1min()
                if replay_1:
                    Xr1, yr_exh = replay_1
                    Xr1 = Xr1.replace([np.inf, -np.inf], np.nan).dropna()
                    yr_exh = yr_exh.loc[Xr1.index]
                    sp1 = int(len(Xr1) * 0.8)
                    model_3_fresh = ExhaustionModel()
                    model_3_fresh.train(Xr1.iloc[:sp1], yr_exh.iloc[:sp1], Xr1.iloc[sp1:], yr_exh.iloc[sp1:])
